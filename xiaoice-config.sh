...
AUTH_REQUIRED="${XIAOICE_AUTH_REQUIRED:-false}"
...
# 获取公网 URL
PUBLIC_URL=$(curl --noproxy "*" -s --max-time 3 http://localhost:4040/api/tunnels 2>/dev/null | grep -o '"public_url":"https://[^"]*' | cut -d'"' -f4 | head -1)
...
if [ "$AUTH_REQUIRED" = "true" ]; then
    ...
    echo -e "  ${CYAN}x-xiaoice-key: ${ACCESS_KEY}${NC}"
else
    echo -e "  当前模式可不传签名头"
fi
echo ""
